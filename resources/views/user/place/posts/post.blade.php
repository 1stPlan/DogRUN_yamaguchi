...
<section class="post">
  <div class="post__inner">
    ...
    <div class="floating__banner">
      ...
      <a href="{{ route('user.post.create', ['id' => $place->id ])  }}">
      ...
      <ul class="post__list" id="post_list">
        ...
        <li class="post__item">
          ...
          <div class="post__item-bottom">
            ...
            @if( $post['like'] == 1)
            <a class="post__item-liked{{$post->id}}" href="javascript:void(0)" onclick="likedestroy({{$post->id}})">
              <i class="far fa-heart"></i>: <span class="post__itemspan{{$post->id}}">{{ $post['like_count'] }}</span>

            </a>
            @else
            <a class="post__item-like{{$post->id}}" href="javascript:void(0)" onclick="postlike({{$post->id}})">
              <i class="far fa-heart"></i>: <span class="post__itemspan{{$post->id}}">{{ $post['like_count'] }}</span>
            </a>
            @endif
          ...
          <div class="post__item-form">
            <a href="#" class="del" onclick="deletePost(this);" data-id="{{ $post->id }}">削除</a>

            <form method="post" action="{{ action('User\Place\PostController@destroy',$post)}}" id="form_{{ $post->id }}">
              {{ csrf_field() }}
              {{ method_field('delete') }}
            </form>
          </div>
